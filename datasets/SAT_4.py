import os
from typing import Any, Callable, List, Optional, Union, Tuple 
from torchvision.datasets import VisionDataset
import numpy as np
from scipy.io import loadmat

class SAT4(VisionDataset):
    """SAT-4 and SAT-6 datasets
    
    Arguments:
        data {root} -- [description]
    
    Raises:
        ValueError -- [description]
        ValueError -- [description]
    
    Returns:
        [type] -- [description]
    """

    classes_sat4 = {"barren land": 0, "trees": 1, "grassland": 2, "none": 3}
    classes_sat6 = {
        "building": 0,
        "barren land": 1,
        "trees": 2,
        "grassland": 3,
        "road": 4,
        "water": 5,
    }
    def __init__(
        self,
        root, 
        image_set="train",
        download=False,
        transform=False,
        target_transform=False, 
        in_channels: int = 3,
        **kwargs
    ):
        self.channels = in_channels 

        if image_set not in ("train", "test"):
            raise ValueError("Argument image_set should be 'train' or 'test'")

        if download:
            download()

        self.root = os.path.expanduser(root)
        self.image_set = image_set 
        self.transform = transform
        self.target_transform = target_transform

        _mat_path = os.path.join(self.root, "sat-4" + "-full.mat")
        self._mat = loadmat(_mat_path)

        self.images, self.targets = self._load_data(self._mat, self.image_set)

    def __getitem__(self, index):
        image = self.images[ :, :, 0:self.channels, index]  #[H x W x C x Index]
        target = np.argmax(self.targets[:, index], axis=0)
        if self.transform:
            image = self.transform(image)
        if self.target_transform:
            target = self.target_transform(target)
        
        #image = image.permute(2, 0, 1).contiguous()  # [H x W x C] -> [C x H x W ]
        #target = target.permute(1, 0).contiguous() # [C x Index] -> [Index x C]
        return image, target

    def __len__(self): 
        return self.images.shape[3]

    def _load_data(self, mat, image_set):
        if image_set == "train":
            images = mat["train_x"]
            targets = mat["train_y"]
        else:
            images = mat["test_x"]
            targets = mat["test_y"] 

        return images, targets

    def download(self):
        pass
