...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.loss(x, x_hat, mean, log_var, log_det).mean()
...
def main(args):
    ...
    filename = '%s_' % dataset \
             ...
             + 'lt%d_' % latent_dim \
             + 'gt%d_' % gate
    if flow in ['planar', 'radial', 'householder']:
        filename += '_%s_' % flow \
                  + 'len%d' % length
    ...
    if dataset == 'mnist':
        ...
        transform = transforms.Compose(
            [transforms.RandomHorizontalFlip(p=0.5),
             transforms.ToTensor()])
        trainset = torchvision.datasets.CIFAR10(root='/home/bbct/wangfan/dataset/',
            train=True, download=True, transform=transform)
